...
class TestDiscoverSessions:
    ...
    def test_valid_hierarchy(self, tmp_path: Path) -> None:
        ...
        assert m0.npy_path.name == "signals_extractedsignals_raw.npy"
        assert m0.mat_path.name == "events.mat"
    ...
    def test_multiple_npy(self, tmp_path: Path, caplog) -> None:
        """FOV with two .npy files is skipped with a warning."""
        _make_fov(
            tmp_path / "phase" / "animal-1F" / "FOV1",
        ...
        assert len(result) == 0
        assert "multiple .npy" in caplog.text
    ...
    def test_mat_with_extractedsignals_excluded(self, tmp_path: Path) -> None:
        ...
        result = discover_sessions(tmp_path)
        assert len(result) == 1
        assert result[0].mat_path.name == "events.mat"
